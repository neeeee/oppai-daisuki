  ...
  try {
    ...
    if (duplicatesByTitle.length > 0) {
      ...
      duplicatesByTitle.forEach((dup, index) => {
        ...
        dup.ids.forEach((id, i) => {
          console.log(`   - ID: ${id}, Slug: ${dup.slugs[i] || 'MISSING'}, Created: ${dup.createdAts[i]}`);
        });
    ...
    if (duplicatesBySlug.length > 0) {
      ...
      duplicatesBySlug.forEach((dup, index) => {
        ...
        dup.ids.forEach((id, i) => {
          console.log(`   - ID: ${id}, Title: "${dup.titles[i]}", Created: ${dup.createdAts[i]}`);
        });
      });
    }
    ...
    if (missingSlugVideos.length > 0) {
      ...
      missingSlugVideos.forEach((video, index) => {
        console.log(`${index + 1}. "${video.title}" (ID: ${video._id}, Created: ${video.createdAt})`);
      });
    }

    // Ask for confirmation before deleting
    if (duplicatesByTitle.length === 0 && duplicatesBySlug.length === 0 && missingSlugVideos.length === 0) {
    ...
    if (duplicatesByTitle.length > 0) {
      ...
      for (const dup of duplicatesByTitle) {
        ...
        const sorted = dup.ids
          .map((id, i) => ({ id, createdAt: dup.createdAts[i], slug: dup.slugs[i] }))
        ...
        if (toDelete.length > 0) {
          ...
          console.log(`   ✓ For "${dup._id}": kept ${toKeep.id}, deleted ${result.deletedCount} duplicate(s)`);
          totalDeleted += result.deletedCount;
        }
      }
    }
    ...
    if (duplicatesBySlug.length > 0) {
      ...
      for (const dup of duplicatesBySlug) {
        ...
        const sorted = dup.ids
          .map((id, i) => ({ id, createdAt: dup.createdAts[i], title: dup.titles[i] }))
        ...
        if (toDelete.length > 0) {
          ...
          console.log(`   ✓ For slug "${dup._id}": kept ${toKeep.id}, deleted ${result.deletedCount} duplicate(s)`);
          totalDeleted += result.deletedCount;
        }
      }
    }
