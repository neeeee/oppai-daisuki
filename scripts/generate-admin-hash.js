...
async function main() {
  ...
  if (choice === '1') {
    ...
    if (confirm !== 'y' && confirm !== 'yes') {
      console.log('❌ Setup cancelled. Please save the password before continuing.');
      process.exit(1);
    }
  } else if (choice === '2') {
    password = await new Promise((resolve) => {
      rl.question('\nEnter your password (input will be hidden): ', (answer) => {
        resolve(answer);
      });
    });
  ...
  try {
    ...
    console.log('Add the following environment variables to your .env.local file:\n');
    ...
    console.log('# ALLOWED_ADMIN_IPS="127.0.0.1,::1"  # Comma-separated list of allowed IPs');
    console.log('# STRICT_IP_CHECK="true"              # Enforce IP consistency for sessions');
