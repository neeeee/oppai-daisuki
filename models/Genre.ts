...
// Pre-deleteOne middleware to clean up references
GenreSchema.pre(
  "deleteOne",
...
// Pre-findOneAndDelete middleware to clean up references
GenreSchema.pre("findOneAndDelete", async function (next) {
  try {
    ...
    if (docToDelete) {
      // Remove from parent's subGenres array
      if (docToDelete.parentGenre) {
        await mongoose
          .model("Genre")
          .findByIdAndUpdate(docToDelete.parentGenre, {
            $pull: { subGenres: docToDelete._id },
          });
      }
